...
| 文件 | 功能 |
|---|---|
| `src/em_core.py` | 传感布局、等效源场计算、传播矩阵、反演、远场和指标 |
| `src/run_baseline.py` | 早期接口基线：布局、重建、测点优化、识别 |
| `src/prepare_cst_templates.py` | 生成 CST 测点和导出模板 |
| `src/check_cst_export.py` | 校验 CST nearfield/farfield CSV |
| `src/run_cst_reconstruction.py` | CST 数据导入、等效源重建、远场真值对比 |
| `src/run_cst_recognition.py` | 从 CST nearfield 表格提取空-频-极化特征并训练识别模型 |
| `src/prepare_cst_level2_manifest.py` | 生成 Level 2 多源多状态 CST 执行清单和源配置表 |
| `src/run_cst_recognition_ablation.py` | 对 CST 格式识别数据做测点/频点删减验证 |
| `src/merge_cst_level2_exports.py` | 合并 Level 2 批量 CST 导出并生成缺漏/完整性报告 |
| `src/run_reconstruction_robustness.py` | 扫描噪声、测点比例和正则化参数对重建精度的影响 |
| `src/build_scorecard.py` | 自动汇总评分项证据、关键指标和最终缺口 |
| `src/build_submission_index.py` | 自动生成最终提交包索引和打包缺口清单 |
| `src/normalize_cst_complex_columns.py` | 将 CST 幅值/相位导出归一化为实部/虚部复数字段 |
| `src/prepare_cst_level1_manifest.py` | 生成 Level 1 标准源 CST 执行清单、源参数和验证门槛 |
| `src/merge_cst_level1_exports.py` | 合并并审计 Level 1 标准源 CST 导出，生成缺漏报告和重建队列 |
| `src/run_cst_level1_batch_reconstruction.py` | 对已完整 Level 1 标准源案例批量执行等效源重建并聚合指标 |
| `src/prepare_cst_level1_workpack.py` | 生成半球面 Level 1 CST 建模任务包、导出核对表和单案例任务卡 |
| `src/prepare_cst_level2_workpack.py` | 生成半球面 Level 2 多源多状态 CST 建模任务包、频点任务和单样本任务卡 |
...
实际 CST 导出可能给出幅值/相位而不是实部/虚部。为避免相干重建阶段丢失相位信息，当前新增 `src/normalize_cst_complex_columns.py`：
...
已完成幅相兼容性验证：从 CST 模板数据反造幅值/相位输入，再转回标准复数字段。转换后的数据通过 nearfield/farfield/pair 校验，并可直接接入 `src/run_cst_reconstruction.py`。当前转换链路验证指标：
...
当前新增 `src/run_reconstruction_robustness.py`，在三源参考场上扫描：
...
当前已加入 `src/run_cst_recognition_ablation.py`，用于回答两个问题：
...
为约束 element-library superposition 与复杂载体安装效应之间的偏差，本轮新增 `src/run_cst_structure_comparison.py`。该脚本没有声称完成 full-wave CST airframe 求解，而是在当前 Level 2 CST-derived nearfield/farfield 上施加透明、可复现的机身/机翼/尾翼遮挡迁移函数，再重新统计方向图变化和跨域识别准确率。
...
- 脚本：`src/merge_cst_level1_exports.py`
- 当前合并报告：`outputs/cst_level1_merge_report`
- 当前状态：required 严格范围已完成，必做 2 个案例均完整；建议/可选对照未作为本轮最终门槛。
- 必做验收命令：`python src\merge_cst_level1_exports.py --strict`
- 全量验收命令：`python src\merge_cst_level1_exports.py --strict-all`
- 批量重建命令：`python src\run_cst_level1_batch_reconstruction.py`
...
- 脚本：`src/merge_cst_level2_exports.py`
...
- 验收命令：`python src\merge_cst_level2_exports.py --strict`
...
当前新增 `src/build_scorecard.py`，从现有文档、代码输出和指标文件自动生成评分项证据板：
...
当前新增 `docs/final_submission_package_plan.md` 和 `src/build_submission_index.py`，用于约束最终提交包结构：
...
当前状态：submission 草稿包可由 `src/build_submission_draft.py` 重新生成，submission index 可由 `src/build_submission_index.py` 刷新；正式打包前的主要缺口已收敛为最终报告、PPT、视频导出，以及 G5 风险说明闭合。
...
6. 重跑 `src/build_scorecard.py`、`src/build_problem_requirements_matrix.py`、`src/build_submission_index.py`、`src/build_completion_audit.py`、`src/build_master_dashboard.py` 和 `src/build_submission_draft.py`。
7. 运行 `src/build_progress_report.py --note "G5 最终文件和风险说明已收口"`，确认 `completion_proven=true` 后再进入最终提交。

...
| `recognition_stress_tests/` | Level 2 识别鲁棒性小型结果表，覆盖 G2 代表布局的 clean-train/perturbed-test、增强训练、未见误差族、种子稳定性、缺测缓解、结构化缺测、仪器误差和复合压力对照。 |
...
| `cst_true_nearfield_workpack/operator_packet/` | `python code\prepare_cst_true_nearfield_operator_packet.py` | GitHub 可追踪的 required full-grid 真 monitor CST 任务卡、manifest、验收命令和当前 `0/2` 状态摘要。 |
...
| `recognition_stress_tests/level2_instrument_error/` | `python code\run_cst_recognition_instrument_error.py` | G5 correlated instrument calibration error check for gain drift, sensor bias, frequency slope, polarization imbalance, and mixed amp/phase bias. |
| `recognition_stress_tests/level2_compound_stress/` | `python code\run_cst_recognition_compound_stress.py` | G5 severe compound stress check combining instrument bias with structured sensor-node, polarization-pair, and azimuth-sector dropout. |
...
`recognition_stress_tests/level2_instrument_error/` is generated by
`python code\run_cst_recognition_instrument_error.py`. It compares clean-train
and known-perturbation-augmented training profiles under simulated correlated
instrument calibration errors: global gain drift, per-sensor gain bias,
frequency-response slope, polarization gain imbalance, and mixed amplitude/
phase bias. Current result: all 150 rows pass `0.85`; the worst single row is
`geometric_farthest_32/sensor_gain_bias_3db` at accuracy `0.933`; both training
profiles have mean accuracy about `0.999`. This adds an instrument-like
calibration-error check but is still internal Level 2 CST-derived evidence.

`recognition_stress_tests/level2_compound_stress/` is generated by
`python code\run_cst_recognition_compound_stress.py`. It keeps the known
perturbation augmentation training set and tests unseen severe combinations of
instrument-like gain/phase bias with structured sensor-node, polarization-pair,
and azimuth-sector dropout. Current result: all 240 rows run, but not all rows
pass `0.85`; zero-fill and mask-only expose the compound-error boundary, with
the worst row at `0.733` for
`full_grid_162/zero_fill/sensor_gain3db_sensor_node_dropout25pct`. The best
overall strategy is frequency/sensor median imputation, with mean accuracy
about `0.993` and minimum accuracy about `0.867`. This makes imputation the
current reportable G5 mitigation candidate under severe internal compound
stress, while preserving the boundary that true monitor data and full-wave
airframe validation are still separate gates.

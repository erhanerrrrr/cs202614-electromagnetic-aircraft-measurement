...
| `operator_packet/` | GitHub-tracked CST operator task cards, required full-grid manifest, acceptance commands, and current packet summary generated by `python code\prepare_cst_true_nearfield_operator_packet.py`. |
...
For GitHub task assignment, refresh the focused operator packet with:

```powershell
python code\prepare_cst_true_nearfield_operator_packet.py
```

The current packet reports required full-grid present `0/2`, so it is an
operator handoff and not physical/vector reduced-layout evidence yet.

...
Purpose: audit the local Huygens CST result package and decide whether `H-Field` probe curves can be converted into the local CSV contract.

## Current Status

- Controller status: `target_contract_complete`
- Sample: `L1_short_dipole_x_1p2G`
- Field kind: `h` / `H-Field`
- Target CSV: `data\cst_exports\level1_meshsafe_huygens_source_family\L1_short_dipole_x_1p2G_level1_local_sphere_r0p35_local_hfield.csv`
- Target CSV exists: `True`
- Expected contract rows: `288`
- Artifact count: `24`
- Nearfield binary present: `True`
- Farfield binary present: `True`
- Exportable local result-tree candidates: `772`
...
If only binary artifacts are present, the next step is to open the short-path project in CST and expose solved local `H-Field` table/result items, or add a CST-API path that reads the binary result through an official result accessor.

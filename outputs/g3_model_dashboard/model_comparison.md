...
- Mesh-safe Huygens is no longer blocked at CST export: both Level 1 E/H local-field paths are loaded, and the best batch branches now use real H-field currents. The current bottleneck is cross-source J-scale/sign stability before the operator is safe to propagate.
...
| trusted_sanity | meshsafe_huygens_real_cst_batch | real_eh_strict_batch_calibration_needed | real_cst_data_chain_not_final_huygens_proof | L1_short_dipole_z_1p2G:strict_pass/eh_love_equivalence_plus_j256/eta=1.000eta0/J=256.0000/H=loaded; L1_halfwave_dipole_z_1p2G:strict_pass/eh_love_equivalence_minus_j96/eta=1.000eta0/J=96.0000/H=loaded | 0.9985 | 9.9532e-04 | 0.00 | 96 | `data\sampling_layouts\cst_meshsafe_huygens_extrapolation_batch\meshsafe_huygens_batch_summary.json` |
...
|---:|---|---|---|---|---|
| 1 | Independent workflow | meshsafe_huygens_physics | Close the real E/H Huygens operator-stability gate: tie the plus/minus convention and J-scale normalization to source geometry, then propagate the accepted rule toward the 13 m shell. | Batch summary reports H-field loaded for all required cases, best branches use real E/H currents, and real_eh_operator_calibration_status is stable_for_current_level1_cases or explained by a geometry rule. |  |

...
|---:|---|---|---|---|---|
| 1 | Independent workflow | meshsafe_huygens_physics | Close the real E/H Huygens operator-stability gate: tie the plus/minus convention and J-scale normalization to source geometry, then test true CST x/y, tilted, off-axis, or multi-source cases before propagating the accepted rule toward the 13 m shell. | Batch summary reports H-field loaded for all required cases, frozen_real_eh_rule_status is accepted, rotation_covariance status is pass, and independent CST source-family rows keep the same frozen rule accepted. |  |

...
|---:|---|---|---|---|---|
| 1 | Independent workflow | meshsafe_huygens_physics | Close the real E/H Huygens operator-stability gate: tie the plus/minus convention and J-scale normalization to source geometry, then propagate the accepted rule toward the 13 m shell. | Batch summary reports H-field loaded for all required cases, best branches use real E/H currents, and frozen_real_eh_rule_status is strict/accepted with a documented geometry rule or real_eh_operator_calibration_status is stable_for_current_level1_cases. |  |

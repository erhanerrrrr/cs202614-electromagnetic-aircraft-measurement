...
| trusted_sanity | meshsafe_huygens_real_cst_batch | region_proxy_batch_pass | real_cst_data_chain_not_final_huygens_proof | L1_short_dipole_z_1p2G:region_shape_pass/outgoing_equivalence_minus; L1_halfwave_dipole_z_1p2G:physics_proxy_pass/electric_only_outgoing | 0.9868 | 0.0141 | 0.00 | 96 | `data\sampling_layouts\cst_meshsafe_huygens_extrapolation_batch\meshsafe_huygens_batch_summary.json` |
...
- `meshsafe_huygens_real_cst_batch`: The mesh-safe route now uses real CST local probe curves for two Level 1 sources and passes the data-chain/region-lobe gate; it still needs H-field or calibrated impedance before being used as final Stratton-Chu/Huygens proof.
...
|---:|---|---|---|---|---|
| 1 | Independent workflow | meshsafe_huygens_physics | Add H-field or calibrated-impedance support for the local Huygens surface, then rerun the real CST two-case batch gate. | Batch summary reports two real CST cases with strict_pass or an explicitly justified physics_proxy_pass under the stricter current estimate. |  |

...
52. CST 真近场 operator packet：`code/prepare_cst_true_nearfield_operator_packet.py` 把 required `full_grid_162` 真 monitor 复跑压缩成 GitHub 可追踪的任务卡、manifest 和验收命令，输出 `data/cst_true_nearfield_workpack/operator_packet/`；当前 required full-grid 仍为 `0/2`，下一步是导出两个 486 行真 monitor CSV。
53. CST 真近场 post-export 决策入口：`code/run_true_nearfield_workflow_decision.py` 汇总 handoff、dropzone、gate 和 G3 dashboard，输出 `outputs/cst_true_nearfield_workflow_decision/`；当前决策是等待两个 required `full_grid_162` 真 monitor CSV，再跑 required gate。
54. GitHub 协作入口：`.github/ISSUE_TEMPLATE/` 已补齐 CST、算法、文档和 bug 四类 issue 模板，`CONTRIBUTING.md` 明确了分支、提交、大文件、true-monitor 校验和结论口径规则，便于队友按统一格式推进任务。
55. G2 采样候选决策矩阵：`code/build_sampling_decision_matrix.py` 输出 `data/sampling_layouts/sampling_decision_matrix/`，把 `full_grid_162` 定为物理参考锚点，`geometric_farthest_32` 定为第一 reduced-layout true-monitor 复跑优先项，`fibonacci_snap_120` 定为保守 120 点交叉验证，`task_driven_32/48` 定为分类压力测试探针。
56. G5 识别鲁棒性压力测试：`code/run_cst_recognition_stress_test.py` 输出 `data/recognition_stress_tests/level2_robustness/`，在 clean-train/perturbed-test 设置下验证噪声、相位抖动和通道缺失；当前单因素扰动基本保持 `1.000`，但组合扰动会低于 `0.85`，需要作为分类泛化边界写入报告口径。
57. G5 扰动增强训练对照：`code/run_cst_recognition_augmented_stress_test.py` 输出 `data/recognition_stress_tests/level2_augmented_robustness/`，在同一 held-out 压力测试上加入已知噪声、相位、缺测和组合扰动增强训练；当前 5 个布局 × 8 个压力场景共 40 行均恢复到 accuracy=`1.000`，可作为已知测量误差族可校准的证据，但不能替代 full-wave 复杂载体验证。
58. G5 未见误差族外推验证：`code/run_cst_recognition_leave_one_family_out.py` 输出 `data/recognition_stress_tests/level2_leave_one_family_out/`，逐类留出 `noise/phase/dropout/combined` 扰动族后再测试；当前 35 行全部高于 `0.85`，但 held-out `dropout_25pct` 在 `geometric_farthest_32` 与 `task_driven_48` 上约为 `0.867`，说明缺测/掉通道仍是最窄裕度。
59. G5 多随机种子稳定性验证：`code/run_cst_recognition_seed_stability.py` 输出 `data/recognition_stress_tests/level2_seed_stability/`，对 held-out `noise/dropout` 做 3 个随机种子重复；当前 60 行全部高于 `0.85`，最窄项仍是 `geometric_farthest_32/dropout/dropout_25pct`，mean accuracy 约 `0.933`、min accuracy 约 `0.867`、裁剪后近似 95% CI 为 `[0.768, 1.000]`。
60. G5 缺测通道缓解验证：`code/run_cst_recognition_dropout_mitigation.py` 输出 `data/recognition_stress_tests/level2_dropout_mitigation/`，在两个最紧 dropout 布局上比较 zero-fill、mask 特征和 frequency/sensor median imputation；当前 48 行全部高于 `0.85`，插补策略将 `geometric_farthest_32/dropout_25pct` 聚合结果从 mean `0.956`、min `0.867` 提升到 mean/min `1.000`。
...
66. G3 source-family reduced-layout 重构验证：`code/run_cst_meshsafe_huygens_reduced_layout.py` 在短 x 真实 CST E/H pilot 上测试 72/48/32/24 点布局；直接删点路径 `0` 个 frozen pass，full-96 局部面重构路径 `12` 个 frozen pass，最小可部署通过项为 `fibonacci_snap_24`，结果写入 `data/sampling_layouts/cst_meshsafe_huygens_source_family_reduced_layout_x/`，阶段说明见 `docs/stage_notes/48_g3_huygens_source_family_reduced_layout_reconstruction.md`。
...
10. 运行 `python code\prepare_cst_true_nearfield_operator_packet.py` 刷新 `data/cst_true_nearfield_workpack/operator_packet/`，把两个 required full-grid CSV 变成可分派给 CST 操作者的任务卡、manifest 和验收命令。
11. CST 文件回填后，先运行 `python code\check_true_nearfield_dropzone.py --required-only --full-grid-only` 预检列名、行数、组件和 sensor 子集，再运行 `python code\run_true_nearfield_gate.py --required-only --candidate full_grid_162`。
12. 运行 `python code\run_true_nearfield_workflow_decision.py` 刷新 `outputs/cst_true_nearfield_workflow_decision/`，用 `true_nearfield_workflow_decision.md` 判断下一步是继续等 CST、跑 required gate、复跑 G3 physical baseline，还是刷新报告口径。
13. 运行 `python code\build_sampling_decision_matrix.py` 刷新 `data/sampling_layouts/sampling_decision_matrix/`，用矩阵安排 reduced-layout 真 monitor 复跑和分类压力测试；该矩阵仍是计划依据，不是 final vector/Huygens 证明。
14. 运行 `python code\run_cst_recognition_stress_test.py` 刷新 `data/recognition_stress_tests/level2_robustness/`，若组合扰动仍低于 `0.85`，下一步应加入噪声/相位/缺测增强训练或 error-aware 特征，而不是直接宣称复杂误差下识别稳健。
15. 运行 `python code\run_cst_recognition_augmented_stress_test.py` 刷新 `data/recognition_stress_tests/level2_augmented_robustness/`；若扰动 profile、随机种子或 Level 2 样本库变化，应同时复跑 clean-train 与 augmented 两个脚本，并在最终报告中区分“已知扰动增强有效”和“未知误差仍需外推验证”。
16. 运行 `python code\run_cst_recognition_leave_one_family_out.py` 刷新 `data/recognition_stress_tests/level2_leave_one_family_out/`；若 held-out dropout 仍是最窄裕度，下一步可做多随机种子置信区间、dropout-aware 特征或传感器缺测插补策略。
...
21. 运行 `python code\run_cst_huygens_rotation_covariance.py` 刷新 `data/sampling_layouts/cst_meshsafe_huygens_rotation_covariance/`；当前 `eh_love_equivalence_minus_j96` 为 `rotation_covariance_strict_pass`，2 个真实 E/H 基准例、9 个刚体旋转、18/18 严格通过。这说明 Huygens 实现具备坐标旋转一致性，但还不能替代独立 CST 源族验证，下一步仍要补 x/y/倾斜/偏置/多源真实 CST 导出。
22. 运行 `python code\prepare_cst_huygens_source_family_workpack.py` 刷新 `data/cst_meshsafe_huygens_source_family_workpack/`；当前工作包为 `source_family_workpack_ready`，包含 6 个可自动化 x/y/off-axis CST 源族 case、576 行 case-scoped local Huygens probe 点和 `next_source_family_commands.csv`。下一步是执行这些 CST 生成/求解/导出命令，再用同一个冻结规则 `eh_love_equivalence_minus_j96` 做无重新调参验证。
23. 运行 `python code\build_cst_source_family_generation_status.py` 刷新 `outputs/cst_meshsafe_huygens_source_family_generation/`；当前状态为 `source_family_projects_generated`，真实 CST API 已生成 6 个 E-field 和 6 个 H-field 源族工程。下一步不再是建模兼容性，而是按 `next_source_family_commands.csv` 运行 solver/export，并检查冻结规则在新源族 E/H CSV 上是否仍被接受。
24. 运行 `python code\build_cst_source_family_solver_status.py` 刷新 `outputs/cst_meshsafe_huygens_source_family_solver_status/`；当前首个 `L1_short_dipole_x_1p2G_efield` solver pilot 为 `source_family_solver_pilot_timed_out`：CST API 启动和 ResultTree 探针项生成正常，但 `-40 dB` 时域收敛与 `1457297` 最大步数在 600 秒窗口内没有完成导出。下一步先修短 x pilot 的 solver-safe 参数或频域/快路径，不要直接跑完剩余 11 个源族求解命令。
25. 运行 `python code\build_cst_source_family_solver_safe_status.py` 刷新 `outputs/cst_meshsafe_huygens_source_family_solver_safe_status/`；当前状态为 `source_family_solver_safe_matched_eh_finished`，短 x 源族 7 条跟踪队列已全部完成：`none=78.7 s`、`efarfield96=114.0 s`、`efield24=343.4 s`、`hfield24=333.3 s`、`efield48=946.5 s`、`efield96=3564.2 s`、`hfield96=3348.3 s`。这说明完整 96 点本地 E/H 探针在本机 CST 上均可跑通，但需要约 56 分钟级别的长窗口。下一步导出匹配 E/H CSV 与 far-field 参考，进入冻结 `eh_love_equivalence_minus_j96` Huygens 规则验证。

...
25. 运行 `python code\build_cst_source_family_solver_safe_status.py` 刷新 `outputs/cst_meshsafe_huygens_source_family_solver_safe_status/`；当前状态为 `source_family_solver_safe_full_efield_finished`，短 x 源族 6 档诊断队列已全部完成：`none=78.7 s`、`efarfield96=114.0 s`、`efield24=343.4 s`、`hfield24=333.3 s`、`efield48=946.5 s`、`efield96=3564.2 s`。这说明完整 96 本地 E-field 探针在本机 CST 上可跑通，但需要约 56 分钟级别的长窗口。下一步补同一样本的 96 点 H-field 长窗口 pilot，再导出匹配 E/H 与 far-field 参考，进入冻结 Huygens 规则验证。

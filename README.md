...
25. 运行 `python code\build_cst_source_family_solver_safe_status.py` 刷新 `outputs/cst_meshsafe_huygens_source_family_solver_safe_status/`；当前状态为 `source_family_solver_safe_ladder_partial`，短 x 源族诊断队列已有 4/6 档完成：`none=78.7 s`、`efarfield96=114.0 s`、`efield24=343.4 s`、`hfield24=333.3 s`。这说明基础求解和角域 far-field probe 可正常完成，完整 96 本地 Cartesian 探针超时更可能来自本地探针数量/探针结果生成开销。下一步执行状态页给出的 `efield48` 命令，再决定是否直接跑 `efield96` 或先调 CST 求解/导出策略。
